...
function renderPage(page) {
    ...
    switch (page) {
        ...
        case "executive-summary":
            ...
            renderExecutiveSummary(contentEl);
            break;
    }
}
...
function renderHistoryTable() {
    ...
    if (state.history.length === 0) {
        ...
        totalEl.textContent = state.historyTotal || 0;
        return;
    }
...
function renderDemoSandbox(container) {
    ...
    fileInput.addEventListener("change", async (e) => {
        ...
        // Clear previous video playback to prevent stale 0:00 player
        ...
        try {
            ...
            if (job.status === "AI_PLAYBACK_ACTIVE" || job.status === "COMPLETED") {
                ...
                if (job.output_video) {
                    ...
                    // Add cache-buster to ensure the browser fetches the new video
                    videoPlayer.src = `http://localhost:8000${job.output_video}?t=${Date.now()}`;
                    videoPlayer.play();
                }
...
function renderExecutiveSummary(container) {
    const template = document.getElementById("executive-summary-template");
    container.innerHTML = '';
    container.appendChild(template.content.cloneNode(true));
    
    // Bind action listeners for recent table later in update
    updateExecutiveSummary();
}

async function updateExecutiveSummary() {
    if (state.currentPage !== "executive-summary") return;
    ...
    try {
        ...
        // Update KPIs
        document.getElementById("exec-total-events").textContent = stats.total_events;
        document.getElementById("exec-active-events").textContent = stats.active_events;
        document.getElementById("exec-resolved-events").textContent = stats.resolved_events;
        
        let roundedRate = Math.round(stats.resolution_rate * 10) / 10;
        document.getElementById("exec-resolution-rate").textContent = `${roundedRate}%`;
        
        const avgEl = document.getElementById("exec-avg-res-time");
        if (stats.avg_resolution_time_sec !== null && stats.avg_resolution_time_sec !== undefined) {
            // Convert to minutes / hours
            let sec = stats.avg_resolution_time_sec;
            if (sec < 60) {
                avgEl.textContent = `${Math.round(sec)}s`;
            } else if (sec < 3600) {
                avgEl.textContent = `${Math.round(sec / 60)}m`;
            } else {
                avgEl.textContent = `${Math.round((sec / 3600) * 10) / 10}h`;
            }
        } else {
            avgEl.textContent = "N/A";
        }
        
        // Helper to draw CSS bars
        const drawBars = (containerId, dataDict) => {
            const container = document.getElementById(containerId);
            container.innerHTML = '';
            
            const entries = Object.entries(dataDict).sort((a, b) => b[1] - a[1]);
            if (entries.length === 0) {
                container.innerHTML = '<div style="color: var(--text-secondary); font-size: 13px;">No data</div>';
                return;
            }
            ...
            entries.forEach(([key, val]) => {
                let displayKey = key;
                if (containerId === "exec-camera-bars") {
                    const cam = state.cameras.find(c => c.id.toString() === key);
                    displayKey = cam ? cam.name : `Camera ${key}`;
                }
                
                const pct = maxVal > 0 ? (val / maxVal) * 100 : 0;
                container.innerHTML += `
                    <div class="bar-row">
                        <div class="bar-label" title="${displayKey}">${displayKey}</div>
                        <div class="bar-track">
        ...
        drawBars("exec-severity-bars", stats.by_severity);
        drawBars("exec-rule-bars", stats.by_rule);
        drawBars("exec-camera-bars", stats.by_camera);
        
        // Recent table
        const tbody = document.getElementById("exec-recent-tbody");
        tbody.innerHTML = '';
        if (!stats.recent_critical || stats.recent_critical.length === 0) {
            tbody.innerHTML = '<tr><td colspan="6" style="text-align: center; color: var(--text-secondary);">No critical events found.</td></tr>';
        } else {
            stats.recent_critical.forEach(event => {
                const tr = document.createElement("tr");
                const date = new Date(event.timestamp).toLocaleString();
                const cam = state.cameras.find(c => c.id === event.camera_id);
                const camName = cam ? cam.name : `Camera ${event.camera_id}`;
            ...
            // Re-bind actions
            document.querySelectorAll('#exec-recent-tbody .view-btn').forEach(btn => {
                btn.addEventListener('click', (e) => {
                    openEventModal(e.target.getAttribute('data-id'));
                });
            });
            document.querySelectorAll('#exec-recent-tbody .ack-btn').forEach(btn => {
                btn.addEventListener('click', (e) => {
                    resolveEvent(e.target.getAttribute('data-id'));
                });
            });
        ...
        console.error("Failed to update executive summary", err);
    }
}
...
async function resolveEvent(eventId) {
    ...
    try {
        ...
        if(response.ok) {
            ...
            renderLiveAlerts();
            renderHistoryTable();
            if (state.currentPage === 'executive-summary') {
                updateExecutiveSummary();
            }
        } else {
            const feedback = document.getElementById('modal-feedback');
...
function handleWebSocketMessage(data) {
    ...
    // Re-render executive summary if active
    if (state.currentPage === 'executive-summary') {
        updateExecutiveSummary();
    }
}
